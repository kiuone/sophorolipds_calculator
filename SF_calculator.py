...
def main():
    ...
    **Estequiometria:**
    ...
    with tab2:
        st.header("Parâmetros - Cálculo Inverso")
        soforo_desejado = st.number_input("Quantidade Desejada de Soforolipídeos (kg)", value=100.0, format="%.2f", key='sd2')
        ...
        if st.button("Calcular Inverso", key='calc2'):
            results = calcular_inverso(soforo_desejado, params_inv, composicao_oleo_inv)
            st.header("Resultados")
            if results['frasco']['volume_excedido']:
                st.warning("Atenção: Volume total de insumos excede o volume do frasco!")
            if results['seed']['volume_excedido']:
            ...
            if results['fermentador']['volume_excedido']:
                st.warning("Atenção: Volume total de insumos excede o volume do fermentador!")
            if results['fermentador']['limitante']:
                st.warning(f"Óleo é limitante! Necessário: {results['fermentador']['oleo_consumido']:,.2f} kg "
                           f"({results['fermentador']['percentual_oleo']:,.1f}% do disponível).".replace(',', 'X').replace('.', ',').replace('X', '.'))
